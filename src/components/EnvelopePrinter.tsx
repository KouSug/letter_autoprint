...
const ENVELOPE_SIZES = {
    chou3: { label: '長形3号 (120x235mm)', width: 120, height: 235 },
    kaku2: { label: '角形2号 (240x332mm)', width: 240, height: 332 },
};

type EnvelopeSizeKey = keyof typeof ENVELOPE_SIZES;

export const EnvelopePrinter: React.FC = () => {
    const [envelopeSize, setEnvelopeSize] = useState<EnvelopeSizeKey>('chou3');
    ...
    const [fontSettings, setFontSettings] = useState({
        zipCode: { family: '"Noto Sans JP", sans-serif', size: 24, x: 0, y: 0, spacing: 3 },
        ...
        <>
                            ...
                            >
                                ...
                                <div className="w-full lg:w-[400px] flex flex-col gap-6 shrink-0 order-2 lg:order-1">
                                    ...
                                    <div className="glass p-6">
                                        ...
                                        <div className="mb-6">
                                            <label className="text-sm text-slate-400 block mb-2">封筒サイズ</label>
                                            <select
                                                value={envelopeSize}
                                                onChange={(e) => setEnvelopeSize(e.target.value as EnvelopeSizeKey)}
                                                className="w-full bg-slate-700 border border-slate-600 rounded p-2 text-sm"
                                            >
                                                {Object.entries(ENVELOPE_SIZES).map(([key, config]) => (
                                        ...
                                        <div style={{
                                            ...
                                            {currentPersonData && (
                                                <EnvelopePreview
                                                    ...
                                                    width={ENVELOPE_SIZES[envelopeSize].width}
                                                    height={ENVELOPE_SIZES[envelopeSize].height}
                                                />
                                            )}
                                        </div>
            ...
            <div className="print-only">
                {data.map((item, index) => {
                    ...
                    return (
                        <div key={index} className="envelope-page text-slate-900"
                            style={{
                                fontFamily: '"Noto Serif JP", serif',
                                width: `${ENVELOPE_SIZES[envelopeSize].width}mm`,
                                height: `${ENVELOPE_SIZES[envelopeSize].height}mm`
                            }}>
